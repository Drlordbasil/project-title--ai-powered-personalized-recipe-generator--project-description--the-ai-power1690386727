...
class RecipeGenerator:
    ...
    def compute_cosine_similarity(self, tfidf_matrix, tfidf_vectorizer, processed_input):
        cosine_sim = linear_kernel(tfidf_matrix, tfidf_vectorizer.transform([processed_input])).flatten()
        return cosine_sim

    def recommend_recipes(self, user_input, preferences):
        processed_input = self.preprocess_input(user_input)

        self.recipes['sentiment_score'] = self.recipes['description'].apply(self.sentiment_score)

        tfidf_matrix, tfidf_vectorizer = self.compute_tfidf_matrix(self.recipes['description'])

        cosine_sim = self.compute_cosine_similarity(tfidf_matrix, tfidf_vectorizer, processed_input)

        self.recipes['score'] = 0.6 * self.recipes['sentiment_score'] + 0.4 * cosine_sim
        recommended_recipes = self.recipes.sort_values(by='score', ascending=False).head(5)
    ...
    def suggest_substitutions(self, ingredient, dietary_restrictions):
        suggested_substitutions = self.substitutions[
            (self.substitutions['ingredient'] == ingredient.lower()) & (self.substitutions['dietary_restriction'].isin(dietary_restrictions))
        ]['substitute']
...
if __name__ == '__main__':
    ...
    recommended_recipes = recipe_generator.recommend_recipes(user_input, preferences)
    ...
    suggested_substitutions = recipe_generator.suggest_substitutions(ingredient, dietary_restrictions)
